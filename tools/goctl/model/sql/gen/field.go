...
func genFieldsPtr(table Table, fields []*parser.Field) (string, error) {
	var list []string

	for _, field := range fields {
		result, err := genFieldPtr(table, field)
		if err != nil {
			return "", err
		}
...
func genFieldPtr(table Table, field *parser.Field) (string, error) {
	tag, err := genTag(table, field.NameOriginal)
	if err != nil {
		return "", err
	}

	text, err := pathx.LoadTemplate(category, fieldPtrTemplateFile, template.FieldPtr)
	if err != nil {
		return "", err
	}
